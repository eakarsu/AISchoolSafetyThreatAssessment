...
// Intake threat reports, AI rates severity based on context
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AISchoolSafetyThreatAssessment'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AISchoolSafetyThreatAssessment. Focus area: Threat severity scoring. ${`Intake threat reports, AI rates severity based on context`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Threat severity scoring.\n${`Intake threat reports, AI rates severity based on context`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'threat-severity', context, parsed);
    res.json({ feature: 'threat-severity', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[threat-severity]', err.message);
...
router.get('/history', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, endpoint, input_data, result, created_at FROM ai_results WHERE endpoint=$1 AND user_id=$2 ORDER BY created_at DESC LIMIT 50', ['threat-severity', req.user.id]);
